...
chart_title = "OLS: Without REER (US-Only)"
fig = heatmap(
...
list_file_names += [file_name]
chart_title = "OLS: With REER (US-Only)"
fig = heatmap(
...
list_file_names += [file_name]
chart_title = "AICs and Log-Likelihood of Estimated Models (US-Only)"
fig = heatmap(
    input=df_loglik,
    mask=False,
    colourmap="vlag",
...
file_name_pdf = path_output + "phillipscurve_urate_ugap_usa_params"
...
telsendmsg(
    conf=tel_config,
    msg="global-plucking --- analysis_phillipscurve_urate_ugap_usa: COMPLETED",
)

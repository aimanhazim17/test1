...
params_ugap_allcountries = pd.DataFrame(columns=["Parameter", "LowerCI", "UpperCI"])
params_ugap_allcountries_reer = pd.DataFrame(
    columns=["Parameter", "LowerCI", "UpperCI"]
)
for country, country_nice in tqdm(zip(list_countries_keep, list_countries_keep_nice)):
    ...
    file_name = path_output + "phillipscurve_ugap_params_ols" + "_" + country
    ...
    file_name = path_output + "phillipscurve_ugap_params_ols_reer" + "_" + country
    list_file_names += [file_name]
...
file_name = path_output + "phillipscurve_ugap_cbyc_params_ols"
list_file_names += [file_name]
fig = heatmap(
    input=params_ugap_allcountries,
    mask=False,
    colourmap="vlag",
    outputfile=file_name + ".png",
    title="Slope of Estimated Phillips Curve (U-Rate Gap) by Countries; Without REER",
    lb=params_ugap_allcountries.min().min(),
    ub=params_ugap_allcountries.max().max(),
    format=".4f",
    show_annot=True,
    y_fontsize=heatmaps_y_fontsize,
    x_fontsize=heatmaps_x_fontsize,
...
# telsendimg(conf=tel_config, path=file_name + ".png", cap=chart_title)
file_name = path_output + "phillipscurve_ugap_cbyc_params_ols_reer"
list_file_names += [file_name]
fig = heatmap(
    input=params_ugap_allcountries_reer,
    mask=False,
    colourmap="vlag",
    outputfile=file_name + ".png",
    title="Slope of Estimated Phillips Curve (U-Rate Gap) by Countries; With REER",
    lb=params_ugap_allcountries_reer.min().min(),
    ub=params_ugap_allcountries_reer.max().max(),
    format=".4f",
    show_annot=True,
    y_fontsize=heatmaps_y_fontsize,
    x_fontsize=heatmaps_x_fontsize,
...
file_name_pdf = path_output + "phillipscurve_ugap_cbyc_params"

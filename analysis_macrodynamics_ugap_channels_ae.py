...
irf_total = pd.read_parquet(path_output + "macrodynamics_ugap_lp_irf_ae" + ".parquet")
...
# Generate output
irf_consol.to_parquet(path_output + "macrodynamics_ugap_lp_irf_channels_ae" + ".parquet")
...
irf_consol = pd.read_parquet(
    path_output + "macrodynamics_ugap_lp_irf_channels_ae" + ".parquet"
)
...
file_name = path_output + "macrodynamics_ugap_lp_irf_channels_ae"
...
for shock in tqdm(endog_base):
    pic_name = path_output + "macrodynamics_ugap_lp_irf_channels_ae" + "_" + shock
    ...
    fig_irf = IRFPlotChannels(
        ...
        maintitle="Responses from " + shock + " Shock" + " (AEs)",
        show_fig=False,
        save_pic=False,
    )
    fig_irf.write_image(
...
telsendmsg(
    conf=tel_config,
    msg="global-plucking --- analysis_macrodynamics_ugap_channels_ae: COMPLETED",
)
